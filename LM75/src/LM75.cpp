...
LM75Sensor::Error LM75Sensor::read(etl::span<uint8_t> i2cData) {
    ...
    return switchErrorRead(HAL_I2C::readRegister<HAL_I2C::PeripheralNumber::TWIHS0>(Lm75Addr, i2cData));
};

LM75Sensor::Error LM75Sensor::write(etl::span<uint8_t> buf) {

    return switchErrorWrite(HAL_I2C::writeRegister<HAL_I2C::PeripheralNumber::TWIHS0>(Lm75Addr, buf));
};
