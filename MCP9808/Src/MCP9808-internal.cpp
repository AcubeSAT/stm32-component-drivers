...
void MCP9808::writeReg(uint8_t addr, uint16_t &data) {
    ...
    HAL_I2C_Master_Transmit(i2c, i2cAddress, txData, 3, 100);
}

void MCP9808::readReg(uint8_t addr, uint16_t &result) {
    uint8_t buffer[2]; // a place to hold the incoming data

    HAL_I2C_Master_Transmit(i2c, i2cAddress, &addr, 1, 100); // transmit the target register's address
    HAL_I2C_Master_Receive(i2c, i2cAddress, buffer, 2, 100); // read the register's contents
