...
void MCP9808::writeReg(uint8_t addr, uint16_t &data) {
    ...
    HAL_I2C_Master_Transmit(i2c, MCP9808_I2C_BUS_ADDR, txData, 3, 100);
}

void MCP9808::readReg(uint8_t addr, uint16_t &result) {
    uint8_t buffer[2]; // a place to hold the incoming data

    HAL_I2C_Master_Transmit(i2c, MCP9808_I2C_BUS_ADDR, &addr, 1, 100); // transmit the target register's address
    HAL_I2C_Master_Receive(i2c, MCP9808_I2C_BUS_ADDR, buffer, 2, 100); // read the register's contents
